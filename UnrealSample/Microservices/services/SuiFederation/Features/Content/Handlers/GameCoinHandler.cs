...
public class GameCoinHandler : IService, IContentHandler
{
    ...
    public async Task<GameCoinMintMessage> PositiveAmountMessage(string transaction, string wallet, string function, InventoryRequest inventoryRequest)
    {
    ...
    public async Task SendMessages(string transaction, string wallet, List<BaseMessage> messages)
    {
        ...
        {
            await SendPositiveAmountMessage(transaction, mintMessages.Cast<GameCoinMintMessage>().ToList());
        }

        var burnMessages = lookup[typeof(GameCoinBurnMessage)].ToList();
        if (burnMessages.Count > 0)
        {
            await SendNegativeMessage(transaction, burnMessages.Cast<GameCoinBurnMessage>().ToList());
        }
    }

    public async Task SendPositiveAmountMessage(string transaction, List<GameCoinMintMessage> messages)
    {
        var transactionManager = _transactionManagerFactory.Create(transaction);
        try
        {
    ...
    private async Task SendNegativeMessage(string transaction, List<GameCoinBurnMessage> messages)
    {
        ...
        try
        {
            ...
            var burnResult = result is SuiTransactionResult transactionResult ? transactionResult : default;
            await transactionManager.AddChainTransaction(new ChainTransaction
            {
                Digest = burnResult.digest,
            ...
            {
                var message = $"{nameof(GameCoinHandler)}.{nameof(SendNegativeMessage)} failed with status {burnResult.status}";
                BeamableLogger.LogError(message);
                await transactionManager.TransactionError(transaction, new Exception(message));
            }
        }
