...
public class RegularCoinHandler : IService, IContentHandler
{
    ...
    public async Task<RegularCoinMintMessage> PositiveAmountMessage(string transaction, string wallet, string function, InventoryRequest inventoryRequest)
    {
    ...
    public async Task SendMessages(string transaction, string wallet, List<BaseMessage> messages)
    {
        ...
        {
            await SendPositiveAmountMessage(transaction, mintMessages.Cast<RegularCoinMintMessage>().ToList());
        }
    ...
    public async Task SendPositiveAmountMessage(string transaction, List<RegularCoinMintMessage> messages)
    {
        var transactionManager = _transactionManagerFactory.Create(transaction);
        try
        {
    ...
    private async Task SendNegativeMessage(string transaction, List<RegularCoinBurnMessage> messages)
    {
        ...
        try
        {
            ...
            {
                var message = $"{nameof(RegularCoinHandler)}.{nameof(SendNegativeMessage)} failed with status {burnResult.status}";
                BeamableLogger.LogError(message);
                await transactionManager.TransactionError(transaction, new Exception(message));
            }
        }
