...
public class CoinCurrencyHandler : IService, IContentContractHandler
{
    ...
    public async Task HandleExistingContract(ContentContractsModel model)
    {
        try
        {
            var clientContentInfo = model.Parent;
            var contract = await _contractService.GetByContent<CoinContract>(clientContentInfo.Id);
            if (contract is null || clientContentInfo is not CoinCurrency coinCurrency)
    ...
    private async Task CompileContract(CoinCurrency coinCurrency, string existingPackageId = "")
    {
        await _suiClient.Compile(coinCurrency.ToModuleName(), existingPackageId: existingPackageId);
    }

    private async Task PublishContract(CoinCurrency coinCurrency)
    {
