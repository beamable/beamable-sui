...
public class AccountsService : IService
{
    ...
    {
        return await GlobalCache.GetOrCreate<Account>($"account-{accountName}", async _ =>
        {
    ...
    public async Task<string> GetWallet(long gamerTag, MicroserviceInfo microserviceInfo)
    {
        var account = await SearchAccount(gamerTag.ToString());
        if (account is null)
        {
            BeamableLogger.LogError("Account not found for gamerTag {gt}", gamerTag);
            throw new UnknownAccountException($"Account not found for gamerTag {gamerTag}");
        }

        var wallet = account.external
            .Where(x => x.providerService == microserviceInfo.MicroserviceName && x.providerNamespace == microserviceInfo.MicroserviceNamespace)
            .Select(x => x.userId)
            .FirstOrDefault();

        if (string.IsNullOrWhiteSpace(wallet))
        {
            BeamableLogger.LogError("Can't find a wallet for gamerTag {gt} for nameSpace {ns}", gamerTag, microserviceInfo.MicroserviceNamespace);
            throw new UnknownAccountException($"Account not found for gamerTag {gamerTag}");
        }
        return wallet;
    }
