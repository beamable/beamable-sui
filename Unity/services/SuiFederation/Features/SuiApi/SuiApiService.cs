...
public class SuiApiService : IService
{
    ...
    public async Task<ISuiTransactionResult> TransferSui(string toAddress, long amount)
    {
        using (new Measure($"Sui.TransferSui to: {toAddress}"))
        {
            try
            {
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var environment = await _configuration.SuiEnvironment;
                var result = await NodeService.TransferSui(toAddress, amount, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
    ...
    public async Task<ISuiTransactionResult> CreatePersonalKiosk(PersonalKioskCreateMessage message)
    {
        using (new Measure("Sui.CreatePersonalKiosk"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskCreate(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"CreatePersonalKiosk: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskList(PersonalKioskListMessage message)
    {
        using (new Measure("Sui.PersonalKioskList"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskList(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskList: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskDelist(PersonalKioskDelistMessage message)
    {
        using (new Measure("Sui.PersonalKioskDelist"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskDelist(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskDelist: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskTake(PersonalKioskTakeMessage message)
    {
        using (new Measure("Sui.PersonalKioskTake"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskTake(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskTake: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskDeclinePurchase(PersonalKioskDeclinePurchaseMessage message)
    {
        using (new Measure("Sui.PersonalKioskDeclinePurchase"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskDeclinePurchase(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskDeclinePurchase: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskCancelExclusive(PersonalKioskCancelExclusiveMessage message)
    {
        using (new Measure("Sui.PersonalKioskCancelExclusive"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskCancelExclusive(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskCancelExclusive: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskPurchase(PersonalKioskPurchaseMessage message)
    {
        using (new Measure("Sui.PersonalKioskPurchase"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskPurchase(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskPurchase: {ex.Message}");
            }
        }
    }

    public async Task<ISuiTransactionResult> PersonalKioskWithdraw(PersonalKioskWithdrawMessage message)
    {
        using (new Measure("Sui.PersonalKioskWithdraw"))
        {
            try
            {
                var environment = await _configuration.SuiEnvironment;
                var realmAccount = await _accountsService.GetOrCreateRealmAccount();
                var requestJson = JsonSerializer.Serialize(message);
                var result = await NodeService.PersonalKioskWithdraw(requestJson, realmAccount.PrivateKey, environment);
                return JsonSerializer.Deserialize<ISuiTransactionResult>(result, _suiTransactionSerializerOptions)
                       ?? throw new SuiApiException("Deserialization failed.");
            }
            catch (Exception ex)
            {
                throw new SuiApiException($"PersonalKioskWithdraw: {ex.Message}");
            }
        }
    }
}