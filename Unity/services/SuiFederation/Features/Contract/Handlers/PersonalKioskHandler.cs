...
public class PersonalKioskHandler : IService
{
    ...
    private const string ModuleName = "personalkiosk";

    public PersonalKioskHandler(ContractService contractService, SuiClient suiClient, SuiApiService suiApiService, Configuration configuration)
    {
    ...
    public async Task<PersonalKioskContract> GetContract()
    {
        return (await _contractService.GetByContent<PersonalKioskContract>(ModuleName))!;
    }
    ...
    public async Task HandleContract()
    {
        ...
        var contract = await _contractService.GetByContent<PersonalKioskContract>(ModuleName);
        if (contract != null)
        {
    ...
    {
        var itemTemplate = await File.ReadAllTextAsync("Features/Contract/Templates/personal_kiosk.move");
    ...
    private async Task<string> PublishContract(PersonalKioskContractModel itemContent)
    {
        ...
        await _contractService.UpsertContract(new PersonalKioskContract
        {
            PackageId = packageId,
