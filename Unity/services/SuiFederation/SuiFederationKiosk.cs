...
public partial class SuiFederation
{
    ...
    {
        var usePersonalKiosk = await Provider.GetService<Configuration>().UsePersonalKiosk;
        if (usePersonalKiosk)
            await Provider.GetService<PersonalKioskListEndpoint>().PersonalKioskList(itemId, price,0);
        else
    ...
    public async Promise DelistFromSale(string listingId)
    {
        var usePersonalKiosk = await Provider.GetService<Configuration>().UsePersonalKiosk;
        if (usePersonalKiosk)
            await Provider.GetService<PersonalKioskDelistEndpoint>().PersonalKioskDelist(listingId, true);
        else
