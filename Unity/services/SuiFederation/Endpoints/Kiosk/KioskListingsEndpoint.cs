    ...
    public async Task<KioskListingsResponse> OwnedListings(string optionalKioskContentId)
    {
        ...
        return new KioskListingsResponse
        {
            listings = listings.Select(l => new KioskListing
            {
                ...
                exclusiveBuyer = l.ExclusiveBuyer != null ? new KioskListingExclusiveBuyer
                {
                    gamerTag = l.ExclusiveBuyer?.GamerTag ?? 0,
                    wallet = l.ExclusiveBuyer?.Wallet ?? "",
                    purchaseCap = l.ExclusiveBuyer?.PurchaseCap ?? ""
                } : null
            }).ToList()
        };
    }
