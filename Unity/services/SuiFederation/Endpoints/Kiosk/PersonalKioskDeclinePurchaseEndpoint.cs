...
public class PersonalKioskDeclinePurchaseEndpoint : IEndpoint
{
    ...
    public async Task PersonalKioskDeclinePurchase(string listingId)
    {
        ...
        var transactionId = await _transactionManager.StartTransaction(wallet, nameof(PersonalKioskDeclinePurchase), kioskTransactionId, new {wallet, listingId});
        ...
        _ = _transactionManager.RunAsyncBlock(transactionId, kioskTransactionId, async () =>
        {
            await ChannelService.Enqueue(_requestContext.UserId, async (_) =>
                await _kioskService.PersonalKioskDeclinePurchase(new PersonalKioskDeclinePurchaseModel(_requestContext.UserId, listingId, wallet, itemContract, kioskContract, validationResult.Seller, validationResult.PurchaseCap, transactionId.ToString(), validationResult.Identity.MicroserviceNamespace))
            );
    ...
    private async Task<ValidationResult> ValidateRequest(long gamerTag, string listingId)
    {
        ...
        if (listing.ExclusiveBuyer.GamerTag != gamerTag)
            throw new KioskException($"Listing {listingId} is not for you.");

        return new ValidationResult(listing.ItemContentId, listing.ItemProxyId, listing.ExclusiveBuyer.PurchaseCap, listing.Wallet, new MicroserviceInfo(SuiFederationSettings.MicroserviceName, listing.Namespace));
    }

    private readonly record struct ValidationResult(string ItemContentId, string ItemProxyId, string PurchaseCap, string Seller, MicroserviceInfo Identity);
}