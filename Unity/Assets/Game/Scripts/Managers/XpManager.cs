...
namespace MoeBeam.Game.Scripts.Sui
{
    public class XpManager : GenericSingleton<XpManager>
    {
        ...
        private void CheckForLevelUp(int xpReward, long instanceId)
        {
            ...
            var currentXp = 0;
            var weapon = BeamInventoryManager.Instance.GetItemByInstanceId(instanceId);
        ...
        private async UniTask LevelUp(WeaponInstance weapon)
        {
            ...
            if(weapon.MetaData.Level < xpGainData.maxLevel)
            {
                ...
                // Every 2 levels, level up creature
                if(nextLevel % 2 == 0)
                    LevelUpCreature(weapon);
        ...
        private void LevelUpCreature(WeaponInstance weapon)
        {
            var creature = BeamContentManager.Instance.GetCurrentCreature();
            if(creature == null) return;
            var biomeNextLevel = creature.CurrentBiome.Level + 1;
            var dnaNextLevel = creature.CurrentDna.Level + 1;
            
            var biomeProperties = new Dictionary<string, string>
            {
                {"$level", biomeNextLevel.ToString()}
            };
            var dnaProperties = new Dictionary<string, string>
            {
                {"$level", dnaNextLevel.ToString()}
            };
            
            var updatedBuilder = new InventoryUpdateBuilder();
            updatedBuilder.UpdateItem(creature.CurrentBiome.ContentId, creature.CurrentBiome.InstanceId, biomeProperties);
            updatedBuilder.UpdateItem(creature.CurrentDna.ContentId, creature.CurrentDna.InstanceId, dnaProperties);
            BeamManager.BeamContext.Api.InventoryService.Update(updatedBuilder).Then(s =>
            {
                weapon.MetaData.ScaleDamageByCreatureBiome(biomeNextLevel);
                BeamContentManager.Instance.LevelUpCreatureBiomeAndDna(biomeNextLevel, dnaNextLevel);
                Debug.Log("Updated Creature Biome and DNA Levels");
            });
        }
