...
{
    public class SuiSlushManager : MonoBehaviour
    {
        [Header("Account Info")] 
        ...
        {
            if (!BeamAccountManager.Instance.IsReady) return;
            Init();
        }
        ...
        private void SetupAccount()
        {
            ...
            if (BeamAccountManager.Instance.CurrentAccount.ExternalIdentities.Length < 1) return;

            if (BeamAccountManager.Instance.CurrentAccount.ExternalIdentities.All(identity =>
                    identity.providerNamespace != SuiFederationSettings.SuiExternalIdentityName)) return;
            // Check if there is an identity for a Slush External Wallet
            _stashedWalletConnected = true;
